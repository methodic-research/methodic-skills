...
description: |
  ...
  that already lives at a gs:// or s3:// URI, author or correct its searchable
  metadata layer, or list and filter the dataset catalog. Example requests:
  "register the dataset already in gs://...", "catalog this turbulence corpus",
  "describe this dataset's PDE, boundary conditions, and variables", "make this
  dataset searchable", "fix the dataset metadata", "what fp64 Navier-Stokes
  datasets do we have", or "list datasets with 3 spatial dims". This is
  register-by-reference only: no local byte upload. For uploading or downloading
  local bytes use chronicle-dataset; for reports use chronicle-write-report; for
  variation config changes use chronicle-author-variation.
---
