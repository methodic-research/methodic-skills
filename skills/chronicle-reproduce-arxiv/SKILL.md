...
description: |
  ...
  reproduction-framed research prompt — all locally, in this session. Do not
  invoke to merely CITE a paper (that's `chronicle-publications`), to
...
framed around it — the whole flow runs locally in this session.

## Transport — MCP-direct, plus the import core

Registration and linking are **bundled MCP tools**
(`chronicle.register_publication`, `chronicle.link_asset`). The
clone-and-import half composes
...
3. **Clone and import.** `git clone` the confirmed
...
3. Everything `chronicle-import-repo`'s hand-off covers (experiment id/URL,
   code binding, report + prompt ids, next steps).
